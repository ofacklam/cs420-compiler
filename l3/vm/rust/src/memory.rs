...
impl Memory {
    ...
    pub fn allocate_lb_ob(&mut self,
                          l_size: L3Value,
                          o_size: L3Value,
                          roots: &mut BaseRegisters) {
        debug_assert!(l_size > 0 && o_size > 0);

        roots.lb = self.allocate(TAG_REGISTER_FRAME, l_size, roots);
        roots.ob = self.allocate(TAG_REGISTER_FRAME, o_size, roots);
    }
