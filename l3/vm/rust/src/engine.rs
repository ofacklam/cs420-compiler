...
use crate::memory::BaseRegisters;
...
enum RegBank { Cb, Ib4, Lb, Lb32, Lb64, Lb96, Lb128, Ob4 }
...
impl Engine {
    pub fn new(mem: Memory) -> Engine {
        Engine { base_regs: [0, 4, 0, 0, 0, 0, 0, 4], mem: mem }
    }

    fn ib(&self) -> usize { self.base_regs[RegBank::Ib4 as usize] - 4 }
    fn lb(&self) -> usize { self.base_regs[RegBank::Lb as usize] }
    fn ob(&self) -> usize { self.base_regs[RegBank::Ob4 as usize] - 4 }

    pub fn set_cb(&mut self, v: usize) {
        self.base_regs[RegBank::Cb as usize] = v
    }
    fn set_ib(&mut self, v: usize) {
        self.base_regs[RegBank::Ib4 as usize] = v + 4
    }
    fn set_lb(&mut self, v: usize) {
        self.base_regs[RegBank::Lb    as usize] = v;
        self.base_regs[RegBank::Lb32  as usize] = v + 32;
        self.base_regs[RegBank::Lb64  as usize] = v + 64;
        self.base_regs[RegBank::Lb96  as usize] = v + 96;
        self.base_regs[RegBank::Lb128 as usize] = v + 128;
    }
    fn set_ob(&mut self, v: usize) {
        self.base_regs[RegBank::Ob4 as usize] = v + 4
    }

    fn gc_roots(&self) -> BaseRegisters {
        BaseRegisters {
            ib: self.ib(),
            lb: self.lb(),
            ob: self.ob()
            ...
            match opcode {
                ...
                RALO => {
                    ...
                    if l_size == 0 {
                        ...
                        self.mem.allocate_lb_ob(l_size, o_size, &mut roots);
                        self.set_lb(roots.lb);
                        self.set_ob(roots.ob);
                    }
                    pc += 1;
                }
                BALO => {
                    ...
                    let block_ix =
                        self.mem.allocate(tag, size, &self.gc_roots());
                    let ra_ix = self.ra_ix(inst);
