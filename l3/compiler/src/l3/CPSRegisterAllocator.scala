...
/**
  ...
  *   I4..I35 contain arguments 1 to 32
  *   O4      contains return value (copied by RET instruction)
...
object CPSRegisterAllocator extends (S.LetF => R.LetF) {
  ...
  private val O4 = R.Reg(ASMRegisterFile.out(4))
    ...
    if (s.retConts(cnt.name))
      s.withFreshRegFor(cnt.args.head, cnt.body) { (r, s) =>
        R.Cnt(R.Label(cnt.name),
              Seq(O4),
              R.LetP(r, CPS.Id, Seq(R.AtomN(O4)), rewrite(cnt.body, s))) }
    else
